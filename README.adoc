...
of values defined when mutlple axes are defined. An `axes` defines a name, in the above example `OS`, with a number of
values, from the abouve example `Linux`, `Windows` and `Mac OS X`.
...
The library is compatible with TeamCity 2019.2.
